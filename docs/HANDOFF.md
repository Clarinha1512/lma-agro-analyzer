...
- [x] **2.1 — DCF simplificado.** Feito: `computeDcfSimplificado` em `indicators.js` (EBITDA como proxy de fluxo de caixa, projeção + valor terminal via Gordon Growth, subtrai dívida líquida). Seção nova na Análise Individual com 4 premissas editáveis (crescimento pré-preenchido pelo CAGR histórico, WACC, anos, crescimento na perpetuidade) e comparação com preço de mercado atual quando preço/nº de ações estão preenchidos. Também corrigido um problema de espaçamento visual (reset CSS zerava margens; classe `.stack-card` adicionada nos cards com múltiplos blocos empilhados: DCF, Crescimento & Múltiplos, DuPont, Scorecard qualitativo).
- [ ] **1.3 — Expandir número de empresas cobertas.** Próximo item — ainda não iniciado.
- [x] **1.6 — Relatório PDF institucional.** Feito: redesenho do output de `window.print()` na Análise Individual seguindo o padrão de relatórios de equity research (referência: modelos reais do BTG Pactual compartilhados pelo usuário) — cabeçalho de marca, "ficha técnica" (rating, preço atual, preço-alvo do DCF, potencial, mini-tabela de múltiplos), "tese do analista" (veredito + justificativa do membro em destaque) e rodapé com aviso de uso educacional. As explicações pedagógicas (comparação com o veredito do sistema, scorecard qualitativo) ganharam destaque visual próprio (`.nota-pedagogica`, borda tracejada dourada) para ficarem claramente separadas da parte "de mercado" — decisão explícita do usuário. Continua sendo `window.print()` (sem lib de PDF nova), só o HTML/CSS impresso foi reorganizado.
...
## 13. Melhorias de feedback externo (fora do backlog original)

Três melhorias pedidas depois de um feedback externo à liga. O princípio que as guia: **o sistema é formativo, não um gerador de relatórios** — o membro articula o raciocínio dele ANTES de ver qualquer output, e profundidade de raciocínio importa mais que quantidade de métricas (explicitamente: não adicionar novos indicadores quantitativos).

- [x] **1. Tese qualitativa obrigatória.** O botão "Gerar relatório" fica travado até o membro escolher o veredito e responder as três perguntas da tese (mínimo de 300 caracteres somados — `TESE_MIN_CARACTERES` em `analiseIndividual.js`). As respostas são campos separados (`PERGUNTAS_TESE`) que viram um texto único com títulos ao salvar, gravado em `analises.notas_membro`. A gravação acontece **no clique de "Gerar relatório"**, antes de qualquer número aparecer, com `tese_registrada_em` — é isso que dá integridade formativa ao registro (não dá pra editar a tese depois de ver a resposta). O antigo botão "Salvar análise" foi removido: o registro agora é único e acontece na geração. No relatório, bloco "Sua leitura × o que os dados mostram" põe a tese lado a lado com a leitura do sistema.
  - **Leitura escrita do sistema** (`leituraDoSistema` em `indicators.js`): frases montadas por regras a partir dos valores já calculados — score, classificações, tendência vs. período anterior e comparação com a mediana do subsetor. Cada frase só entra se o dado existir (sem período anterior não fala de tendência, sem peers não fala de mediana). Nada estimado, nada inventado, sem chamada de IA.
- [ ] **2. Bloco de externalidades e oportunidades.** Categorias (clima, câmbio, regulação/ambiental, geopolítica/comércio exterior, demanda, tecnologia), cada uma marcada como risco/oportunidade/neutro + justificativa curta, todas opcionais, persistidas junto com a tese e incluídas no relatório.
- [ ] **3. Camada de projeção com os dados históricos existentes.** Tendência dos indicadores ao longo do tempo, sinalização de aceleração/desaceleração/inflexão, comparação com a mediana do setor na mesma janela, e a leitura da projeção escrita pelo próprio membro. ⚠️ **Verificar antes de implementar:** o pedido menciona "histórico desde 2011, trimestral", mas a base tinha 3 períodos anuais (2022-2024) por empresa — confirmar o que existe hoje antes de desenhar o escopo.
